...
**File:** ``src/model_training/train.py`` — run as a script
(not imported for side effects).

**Usage (repo root)** — run EDA preprocess first so the cleaned CSV exists.
...
**Scope (assignment Tasks 2–4).** Loads
``data/heart_disease_processed_dataset.csv``, builds two sklearn ``Pipeline``
objects (Logistic Regression with scaling; Random Forest without scaling),
tunes hyperparameters with stratified ``GridSearchCV`` (ROC-AUC), reports
hold-out test metrics, logs params/metrics/artifacts to MLflow (experiment
``heart_disease_prediction`` by default, store ``./mlruns``), and saves the
**best** model by test ROC-AUC into ``models/``.

**Encoding.** Categorical handling is done upstream in
``data_preprocessing/pre_processing_data.py``; this file only adds
imputation/scaling appropriate to each estimator family.
...
- ``mlruns/`` — MLflow runs (models, plots; UI via
...
def _patch_mlflow_sklearn_pyfunc_if_lzma_missing() -> None:
    """
    MLflow sklearn.save_model calls pyfunc.add_to_model; loading
    mlflow.pyfunc imports lzma. Some Python builds (e.g. pyenv without xz)
    omit _lzma, so log_model fails and the UI shows broken model artifacts or
    an upload-failed state. Stubbing add_to_model skips the pyfunc flavor
    only; sklearn flavor and MLmodel still log correctly.
    """
    try:
...
# Inner CV folds for GridSearchCV / cross_validate (stratified). Test set
# fraction is controlled in prepare_features (default 20% hold-out), separate
# from these folds.
CV_SPLITS = 5
RANDOM_STATE = 42
...
def get_parallel_n_jobs() -> int:
    ...
    ``MLOPS_N_JOBS`` (integer) overrides detection — use ``1`` in CI sandboxes
    or hosts where joblib/loky fails with ``PermissionError`` on startup.

    Otherwise, if ``os.sysconf`` is unavailable or forbidden (common in
    hardened sandboxes), returns ``1`` so training still completes; on normal
    Unix/macOS returns ``-1`` (all CPUs).
    """
...
def load_clean_data(path: Path | str | None = None) -> pd.DataFrame:
    ...
    **Why this exists:** Training assumes preprocessing was done once and
    saved to disk, so experiments stay reproducible and fast. If this file is
    missing, run
    ...
    ----------
    path
        Defaults to :obj:`config.paths.CLEAN_DATA_CSV`
...
def prepare_features(df: pd.DataFrame, test_size: float = 0.2):
    ...
    **Why stratify:** Keeps similar class balance in train and test when
    computing final metrics. The **test** portion is never used during
    ``GridSearchCV``; it only measures how the tuned model generalises.

    All columns except ``target`` are numeric (already encoded upstream in
    preprocessing).
...
def build_logistic_pipeline() -> Pipeline:
    """
    Linear model pipeline: median imputation (defensive) + standardization +
    LR.

    Scaling puts features on comparable scales for regularized logistic
    regression.
    """
    return Pipeline(
        [
...
def _binary_metrics(y_true, y_pred, y_prob) -> dict[str, float]:
    """
    Compute accuracy, precision, recall, F1, and ROC-AUC for binary
    classification.
    """
    return {
...
def run_cross_validation_report(
    ...
    **Purpose:** Summarise how stable metrics are across folds (mean ± std).
    This is separate from the single hold-out **test** set used in
    :func:`evaluate_on_test`. Metrics are logged to MLflow as ``cv_*`` keys.
    """
    cv = StratifiedKFold(
    ...
    print(
        f"   CV (mean ± std): "
        f"ROC-AUC={summary['cv_roc_auc_mean']:.4f}"
        f"±{summary['cv_roc_auc_std']:.4f}, "
        f"accuracy={summary['cv_accuracy_mean']:.4f}"
        f"±{summary['cv_accuracy_std']:.4f}"
    )
    return summary
...
def evaluate_on_test(
    ...
    """
    Evaluate the fitted pipeline on the **held-out test set** and save
    diagnostic plots.

    Returns a dict of metrics (same keys as :func:`_binary_metrics`). Prints
    sklearn's classification report and writes confusion-matrix and ROC PNGs
    under ``output_dir``.
    """
    ...
    _plot_confusion_matrix(
        y_test, y_pred, model_name, output_dir / f"cm_{safe}.png"
    )
    _plot_roc_curve(
        y_test,
        y_prob,
        model_name,
        output_dir / f"roc_{safe}.png",
        metrics["roc_auc"],
    )
    return metrics
...
def _plot_confusion_matrix(
    y_test, y_pred, model_name: str, path: Path
) -> None:
...
def tune_train_log_mlflow(
    ...
    """
    Tune ``pipeline`` over ``param_grid``, log everything to MLflow, return
    winners.
    ...
    **What happens inside**
    ...
    3. Cross-validation summary (:func:`run_cross_validation_report`) and test
       metrics (:func:`evaluate_on_test`) are logged as MLflow metrics.
    4. Confusion/ROC images and the sklearn model artifact are attached to the
       run.
    ...
    -------
    tuple
        ``(best_estimator, test_metrics_dict, cv_summary_dict)`` — test_metrics
        are used later by :func:`save_best_model` to choose between Logistic
        Regression and RF.
    """
    ...
    with mlflow.start_run(run_name=model_name):
        ...
        print(
            f"\nTuning {model_name} (GridSearchCV, {CV_SPLITS}-fold "
        ...
        best_param_log = {
            f"best__{k}": ("null" if v is None else v)
            for k, v in grid.best_params_.items()
        }
        mlflow.log_params({**base_params_for_log, **best_param_log})
        mlflow.log_metric(
            "gridsearch_best_cv_roc_auc", float(grid.best_score_)
        )
        ...
        safe = model_name.replace(" ", "_").lower()
        for rel in [
            SCREENSHOTS_DIR / f"cm_{safe}.png",
            SCREENSHOTS_DIR / f"roc_{safe}.png",
        ]:
            if rel.is_file():
        ...
        try:
            ...
            # Some Python builds lack lzma; MLflow pyfunc may fail. Fallback:
            # pickle artifact.
            warnings.warn(
                f"mlflow.sklearn.log_model failed ({exc!r}); "
                "logging joblib pickle instead."
            )
...
def save_best_model(
    ...
    """
    Pick the winning estimator by **test ROC-AUC** and write deployment-ready
    artifacts.

    **Selection rule:** Compare ``roc_auc`` in each candidate's test metrics
    dict; the highest wins (tie-breaking is arbitrary but rare). Saves:

    - ``models/best_model.pkl`` — joblib-serialised winning ``Pipeline``.
    - ``models/feature_names.pkl`` — column order of ``X_train`` (must match
      prediction).
    - ``models/training_metadata.json`` — JSON with metrics, library
      versions, paths.
...
def main(argv: list[str] | None = None) -> None:
    """
    Entry point: train both models, log to MLflow, save best pipeline under
    ``models/``.

    Expects cleaned training data from EDA. Does not start the MLflow UI
    server.
    """
    parser = argparse.ArgumentParser(
        description=(
            "Train Logistic Regression and Random Forest pipelines, compare "
            "test ROC-AUC, log runs to MLflow, and save the best estimator to "
...
""",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to heart_disease_processed_dataset.csv — numeric features + "
            f"target column (default: {CLEAN_DATA_CSV})"
        ),
    )
    parser.add_argument(
        "--experiment",
    ...
    print(
        ...
        "  • Features are already label-encoded in preprocessing; this step "
        "adds\n"
        "    median imputation (defensive) + StandardScaler for Logistic "
        "Regression.\n"
        "  • Random Forest uses imputation only (trees do not require "
        "scaling).\n"
        "  • Hyperparameters are chosen via GridSearchCV maximising ROC-AUC "
        "on\n"
        "    stratified inner CV; reported metrics include hold-out test "
        "performance.\n"
    )
    ...
    print(
        "\nDone. MLflow UI: mlflow ui --backend-store-uri ./mlruns "
        "(from project root)"
    )
