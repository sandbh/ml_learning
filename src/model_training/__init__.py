...
- ``train.py`` — Builds sklearn pipelines, ``GridSearchCV``, evaluation metrics,
  ...
  metadata to ``models/``.
- ``inference.py`` — Loads ``models/best_model.pkl``, reapplies the same preprocessing
...
**Upstream.** Requires ``data/heart_disease_processed_dataset.csv`` from ``eda/eda.py`` for training and
